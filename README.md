...
- **13 note-accurate reproductions** — each lays a classic electronic track's parts across the two voices: the bassline on ACID-303, the signature riff/hook on LEAD (see below). Plus a **Randomize** button that generates fresh, scale-constrained patterns that always sound musical.
