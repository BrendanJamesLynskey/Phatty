...
- **13 note-accurate reproductions** — each lays a classic electronic track's parts across the two voices: the bassline on ACID-303, the signature riff/hook on LEAD (see below). The melodies are transcribed interval-for-interval from the originals so each hook is instantly recognisable — the octave-drop "pop" of *Popcorn*, the stutter build of *Sandstorm*, the hammered octave pump of *I Feel Love*, the funk syncopation of *Axel F*. Plus a **Randomize** button that generates fresh, scale-constrained patterns that always sound musical.
- **Recognisable at a glance** — a four-colour **voice stripe** (acid · lead · drums · poly) is Phatty's signature mark, carried across the power-on screen, the header wordmark and the landing page; and the **NOW PLAYING** LCD strip announces the current track and its original artist/year so you always know what you're hearing.
...
- **MIDI import** — the **♪ MIDI** button loads any Standard MIDI File (`.mid`), the universal interchange format for sequences. It reads the tempo, maps **General-MIDI channel-10 drums** (36 = kick, 38 = snare, 42 = closed hat, 46 = open hat, 39 = clap, 56 = cowbell, toms…) onto the 808 lanes, the low/bass notes onto the ACID-303, and the melody onto the LEAD (note lengths become ties), quantised to the step grid — so you can load publicly-available or your own MIDI versions of a groove. *(MIDI transcriptions of copyrighted songs still carry that song's copyright; public-domain, freely-licensed, or your own files are the clean sources.)*
